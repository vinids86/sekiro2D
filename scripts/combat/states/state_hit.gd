...
func on_timeout(cc: CombatController) -> void:
	var cfg: AttackConfig = cc.get_current_attack()
	assert(cfg != null, "StateHit.on_timeout: cfg nulo")
	cc._change_state(CombatController.State.RECOVER, cfg, maxf(cfg.recovery, 0.0))
