...
func on_timeout(cc: CombatController) -> void:
	...
	assert(cfg != null, "StateHeavyStartup.on_timeout: cfg nulo")
	cc._change_state(CombatController.State.HEAVY_HIT, cfg, maxf(cfg.hit, 0.0))
